...
in {
  perSystem = {
    ...
    packages.module-docs-md = let
      sources = [
        {
          name = "${self}";
          url = "https://github.com/kolloch/n2c-mod/blob/main";
        }
        {
          name = "${inputs.nixpkgs}";
          url = "https://github.com/NixOS/nixpkgs/blob/main";
        }
        {
          name = "${inputs.flake-parts}";
          url = "https://github.com/hercules-ci/flake-parts/blob/main";
        }
      ];
      rewriteSource = decl: let
        prefix = lib.strings.concatStringsSep "/" (lib.lists.take 4 (lib.strings.splitString "/" decl));
        source =
          lib.lists.findFirst (src: src.name == prefix) {
